          ...
          <Row>
            <Col span="12">
              <FormItem
                ...
                :rules="[{required: true, message: '材料不能为空', trigger: 'change'}]"
              >
...
import { doOriginCompositionAddMany, doOriginCompositionDelMany, findOriginCompositionInfo } from '@/api/menu-origin-composition'
import { doStandardCompositionDelMany, findStandardCompositionInfo } from '@/api/menu-standard-composition'
...
export default {
  ...
  methods: {
    ...
    async dataInit() {
      ...
      if (this.debug) console.log(this.addressList)
    },
    ...
    async editCompositon(obj) {
      ...
      if (obj.compositionIds !== '') {
        // 将原来的材料删除
        await doOriginCompositionDelMany({ ids: obj.compositionIds })
      }
      // 添加编辑后的材料
      let compositionJson = JSON.stringify(obj.composition)
      console.log('添加编辑的材料', compositionJson)
      await doOriginCompositionAddMany({ addList: compositionJson })
    },
    ...
    async getShowDataBySourceData(data) {
      for (let i = 0; i < data.length; i++) {
        // 得到菜谱对应的材料
        let res2 = await findOriginCompositionInfo({ menuCode: data[i].code })
        // 得到材料ids
    ...
    async getMenuIdByCode(code) {
      let res = await findMenuInfo({ code: code })
      if (this.debug) console.log('getMenuIdByCode', res)
      res = res.data.list
      if (res.length === 0) {
        return null
      } else {
    ...
    async uploadImg() {
      let imgs = this.$refs.imgElem.files
      this.loading = true
      this.imgCount = 0
      this.progress = 0
      this.loadingTitle = '正在导入图片'
      this.imgTotal = imgs.length

      for (let i = 0; i < this.imgTotal; i++) {
        await this.uploadAImg(imgs[i])
        this.progress = (100 * i) / this.imgTotal
      }
      this.cancelModal()
    },
    async uploadAImg(img) {
      let name = img.name
      let pos = name.lastIndexOf('.')
      name = name.substring(0, pos)
      let id = await this.getMenuIdByCode(name)
      if (this.debug) console.log('上传图片id', id)
      if (id === null) return
      let upload = await uploadImg({ imgFile: img })
      let url = upload.data.url
      this.doMenuEdit({ id: id, imgUrl: url })
    },
    // 导入Excel
    uploadExcel(file) {
      // 如果当前正在导入，点击导入无效果
    ...
    async addDate(header, datas) {
      ...
      for (var i = 0; i < length; i++) {
        ...
        let data = datas[i]
        // 将导入的数据转化为显示的数据
        data = this.dataUploadToDataShow(header, data)
    ...
    dataUploadToDataShow(header, data) {
      let res = {}
      ...
      if (data['地址'] !== undefined) {
        let id = this.getAddressIdByName(data['地址'].replace(/^\s+|\s+$/g, ''))
    ...
    deleteInvalidDate(data) {
      for (let i = 0; i < data.composition.length; i++) {
        let com = data.composition[i]
        if (com.quantity === undefined) data.composition[i].quantity = ''
        if (com.composition === undefined) data.composition[i].composition = ''
      }
      return data
    },
    // 添加材料
